...
.ifne GBDK_NES_EVERY_SCANLINE_IRQ
    ...
    ldy __lcd_isr_num_calls+1
0$:
    sta *__ESI_write_index
    sty __lcd_isr_buf_length
...
8$:
    sty *.lcd_num_calls ;__lcd_isr_buf_length
    sta *.lcd_buf_end
...
10$:
    ...
    ; Skip wait-for-next-frame if a frame already passed
    cmp *_sys_time_old
    bne _wait_vbl_done_waitForNextFrame_skip
...
_wait_vbl_done_waitForNextFrame_loop:
    ...
    ;
_wait_vbl_done_waitForNextFrame_skip:
    lda *_sys_time
    sta *_sys_time_old
    ;
    rts
