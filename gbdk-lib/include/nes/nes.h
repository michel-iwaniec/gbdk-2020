...
//#define BANKREF(VARNAME) void __func_ ## VARNAME() __naked { \
//__asm \
...
/** Makes UNROM mapper switch the active ROM bank
    @param b   ROM bank to switch to
*/
#define SWITCH_ROM_UNROM(b) \
  _current_bank = (b), *((uint8_t *)0xF800 + (b)) = (b)
...
*/
void set_interrupts(uint8_t flags) OLDCALL;
 ...
 * @return read value
 */
uint8_t get_vram_byte(uint8_t * addr) OLDCALL;
...
*/
void get_bkg_data(uint8_t first_tile, uint8_t nb_tiles, uint8_t *data) OLDCALL;
...
*/
void set_win_1bpp_data(uint8_t first_tile, uint8_t nb_tiles, const uint8_t *data) OLDCALL;
...
/** Copies from Sprite VRAM Tile Pattern data into a buffer
    ...
    should be at least __nb_tiles__ x 16 bytes in size.
*/
void get_sprite_data(uint8_t first_tile, uint8_t nb_tiles, uint8_t *data) OLDCALL;
...
*/
void get_data(uint8_t *data, uint8_t *vram_addr, uint16_t len) OLDCALL;
...
/** Copies arbitrary data from an address in VRAM into a buffer
    ...
    \li VBK_REG=1 indicates the second
*/
void vmemcpy(uint8_t *dest, uint8_t *sour, uint16_t len) OLDCALL;
...
/** Copies a rectangular region of Tile Map entries from a given VRAM Address into a buffer
    ...
    @param x         X Start position in Background Map tile coordinates. Range 0 - 31
    @param y         Y Start position in Background Map tile coordinates. Range 0 - 29
    @param w         Width of area to copy in tiles. Range 0 - 31
    @param h         Height of area to copy in tiles. Range 0 - 29
    @param vram_addr Pointer to source VRAM Address
    @param tiles     Pointer to destination buffer for Tile Map data
